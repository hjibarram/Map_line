...
def lnlike_gauss_Lin(theta, spec, specE , x, xo1, xo2, xo3, lfac12, single):
...
def lnprior_gauss_Lin(theta,At=0.05,dv1t=200,sim=False, single=False):
    ...
    if single:
        ...
        if ((A1 >= 0) and (A1 <=1.8)) and ((A3 >= 0) and (A3 <=0.8)) and ((fac >= 1.0) and (fac <=10.0)) and ((A7 >= Am1) and (A7 < Am2)) and ((fwhm1 >= 100.0) and (fwhm1 <= 800.0)) and ((fwhm2 >= 700.0) and (fwhm2 <= 10500.0)) and ((dv1 >= dv1i) and (dv1 <= dv1s)) and ((dv2 >= dv2i) and (dv2 <= dv2s))  and ((dv3 >= -1000) and (dv3 <= 1000)): 
            return 0.0
        else:
            return -np.inf
    
def lnprob_gauss_Lin(theta, spec, specE, x, xo1, xo2, xo3, At, dv1t, sim, lfac12, single):
