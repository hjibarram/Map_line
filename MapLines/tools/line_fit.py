...
def line_fit_single(file1,file_out,file_out2,name_out2,dir_out='',
    ...
    if len(keys_param) > 0:
        for i in range(len(keys_param)):
            fits_new_cols.extend([fits.Column(name=keys_param[i], format=tools.numpy_to_tform(valu_param[i]), array=valu_param[i])])
    ...
    hlist.update_extend()
    hlist.writeto(file_out2+'T.fits', overwrite=True)
    tol.sycall('gzip -f '+file_out2+'T.fits')
